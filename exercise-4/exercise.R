...
flights <- mutate(flights, time.in.air = arr_delay - dep_delay)
View(flights)

# Sort your data.frame desceding by the column you just created
flights <- arrange(flights, desc(time.in.air))

# Try doing the last 2 steps in a single operation using the pipe operator
flights <- mutate(flights, time.in.air = arr_delay - dep_delay) %>% arrange(desc(time.in.air))


# Make a histogram of the amount of gain using the `hist` command
hist(flights$time.in.air)

# On average, did flights gain or lose time?
avg <- summarise(flights, mean = mean(time.in.air))

# Create a data.frame that is of flights headed to seatac ('SEA'), 
flights.seatac <- flights %>% select(time.in.air, dest) %>% filter(dest == 'SEA')

# On average, did flights to seatac gain or loose time?
mean(flights.seatac$time.in.air, na.rm = TRUE)
## lose, science < 0
